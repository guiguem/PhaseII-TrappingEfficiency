...
double Solenoid::getRadialMagneticField(double x0, double x1, double x2)
{
    ...
    double B = mu0() * current * numberTurns / (4 * TMath::Pi() * length) * TMath::Sqrt(radius / rho);
...
double Solenoid::getLongMagneticField(double x0, double x1, double x2)
{
    ...
    double B = -mu0() * current * numberTurns / (4 * TMath::Pi() * 2 * length) / TMath::Sqrt(radius * rho);
    B *= (tContribPlus - tContribMinus);
...
double Coil::getRadialMagneticField(double x0, double x1, double x2)
{
    ...
    double C = mu0() * numberTurns * current / TMath::Pi();
    ...
    return TMath::Sqrt(TMath::Power(Bx, 2) + TMath::Power(By, 2));
}
...
double Coil::getLongMagneticFieldOnAxis(double z)
{
    ...
    return mu0() * numberTurns * current / 2. * TMath::Power(radius, 2) / TMath::Power(TMath::Power(z - zCoil, 2) + radius * radius, 1.5);
}
